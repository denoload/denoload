...
export function diff<T> (A: T[], B: T[]): Array<DiffResult<T>> {
  ...
  A = (suffixCommon.length > 0)
    ? A.slice(prefixCommon.length, -suffixCommon.length)
    : A.slice(prefixCommon.length)
  B = (suffixCommon.length > 0)
  ...
  const M = A.length
  const N = B.length
  if (!M && !N && (suffixCommon.length === 0) && (prefixCommon.length === 0)) return []
  if (!N) {
    return [
      ...prefixCommon.map(
...
export function diffstr (A: string, B: string) {
  ...
  function tokenize (string: string, { wordDiff = false } = {}): string[] {
    if (wordDiff) {
      ...
      const tokens = []; const lines = string.split(/(\n|\r\n)/)
...
export function buildMessage (
  ...
  const messages: string[] = []; const diffMessages: string[] = []
  messages.push('')
  messages.push('')
  messages.push(
